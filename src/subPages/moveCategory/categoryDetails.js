            ...
            categoryDetails ?
                <div className=' grid grid-cols-1 bg-slate-200 rounded-lg m-2'>
                    ...
                    <div className=' grid grid-cols-3 md:grid-cols-5 lg:grid-cols-9 gap-4 m-2 p-2 rounded-lg bg-slate-100'>
                        {
                            categoryDetails.moves.map((e, i) => <Link key={i} className=' capitalize hover:bg-slate-200 hover:text-blue-500 hover:cursor-pointer hover:underline rounded-lg bg-white p-2 m-2' to={`/t-pokedex/move/${e.name}`}> {e.name}</Link>)
                        }
                    </div>
                </div>
                :
