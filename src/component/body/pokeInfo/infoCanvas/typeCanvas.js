        ...
        <div className='bg-slate-300 rounded-lg items-center m-1'>
            ...
            <div className='bg-white rounded-lg p-2 m-2 flex flex-col md:flex-row items-center justify-center'>
                {types.map((type, index) => (
                    <Link to={`/t-pokedex/type/${type.type.name}`} className={`${typeToColor(type.type.name)} rounded-md text-white text-center text-lg font-semibold border border-indigo-600 m-1 px-2 font-mono hover:cursor-pointer w-24 h-8`} key={index}>
